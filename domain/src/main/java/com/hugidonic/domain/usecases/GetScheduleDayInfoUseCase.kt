...
class GetScheduleDayInfoUseCase(private val scheduleRepository: ScheduleRepository) {
    suspend operator fun invoke(dayOfWeek: String, isFetchFromRemote: Boolean): Flow<Resource<ScheduleDayModel>> {
        return scheduleRepository.getScheduleDayInfo(dayOfWeek = dayOfWeek, isFetchFromRemote = isFetchFromRemote)
    }
}